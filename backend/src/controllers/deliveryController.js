...
exports.getDeliveriesByDate = async (req, res) => {
  try {
    ...
    const subscriptionIds = await subscriptionService.getActiveSubscriptions(userId);
    ...
    const deliveries = await deliveryService.getDeliveriesForADate(userId, new Date(date));
...
exports.rescheduleDelivery = async (req, res) => {
  try {
    ...
    console.log('Rescheduling delivery with ID:', id, 'to new date:', newDate, 'and time:', newTime);
    if (!newDate && !newTime) {
      return res.status(400).json({
        success: false,
    ...
    const updatedDelivery = await deliveryService.rescheduleDelivery(id, newDate, newTime);
